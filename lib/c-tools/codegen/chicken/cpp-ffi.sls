...
;; C++ FFI Code Generator - AST to Chicken Scheme FFI
;; Generates foreign-lambda bindings with C++ name mangling

(library (c-tools codegen chicken cpp-ffi)
  ...
  (define (generate-cpp-ffi-code decls lib-name)
    (let* ([forms (declarations->ffi-forms decls '() lib-name)]
      ...
      (chicken-cpp-module-wrapper lib-name exports forms)))

  ;;=======================================================================
  ;; Module Generation

  (define (chicken-cpp-module-wrapper lib-name exports forms)
    (string-append
      "(module " lib-name " (" (string-join (map symbol->string exports) " ") ")\n"
      "  (import scheme chicken foreign)\n\n"
      "  (foreign-declare \"#include <" lib-name ".hpp>\")\n\n"
  ...
  (define (declarations->ffi-forms decls namespaces lib-name)
    (let loop ([decls decls] [forms '()])
      (if (null? decls)
          ...
          (let ([decl (car decls)])
            (cond
              ;; Namespace - process inner declarations
              [(namespace-decl? decl)
